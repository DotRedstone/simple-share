...
5. **创建并绑定 Cloudflare 资源（重要！必须在部署前完成）**
   ...
   **步骤 4：配置环境变量**
   ...
   - ✅ **绑定配置已自动设置**：`wrangler.toml` 中已配置绑定，**但需要填入实际的 `database_id`**
   - 🔧 **获取 database_id**：
     ```bash
     npx wrangler d1 list
     ```
     或者在 Cloudflare Dashboard → D1 → 你的数据库 → 查看详情
   - 📝 **填入 database_id**：编辑 `wrangler.toml`，将 `YOUR_DATABASE_ID_HERE` 替换为实际的 database_id
   - ✅ **配置后，每次自动部署时绑定不会丢失**

6. **创建 Cloudflare 资源并配置绑定**
   ```bash
   # 创建 D1 数据库
   ...
   **配置 database_id**：
   - 编辑 `wrangler.toml` 文件
   - 找到 `database_id = "YOUR_DATABASE_ID_HERE"`
   - 替换为实际的 database_id（从上面的命令输出或 Dashboard 中获取）
   - 同时更新 `[env.production]` 部分的 database_id（如果使用相同的数据库，可以填相同的 ID）
   
   ✅ **重要**：配置后，每次自动部署时绑定不会丢失
