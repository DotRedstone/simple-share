...
<template>
  <footer class="fixed bottom-0 left-0 right-0 z-40 px-4 py-3 pointer-events-none md:px-8 mb-safe">
    <div class="flex items-center justify-between max-w-screen-2xl mx-auto pointer-events-auto">
      <!-- 左侧：文档与仓库 (仅中大屏显示) -->
      <div class="hidden md:flex items-center gap-4">
        <a 
          href="/docs/REPORT_CONTENT.md" 
          target="_blank"
          class="flex items-center gap-2 px-3 py-1.5 text-xs font-medium rounded-full bg-slate-800/40 hover:bg-slate-700/60 border border-white/5 backdrop-blur-md transition-all text-slate-400 hover:text-white dark:bg-white/5 dark:hover:bg-white/10"
        >
      ...
      <div class="flex items-center gap-3 ml-auto">
        <p class="hidden lg:block text-[10px] text-slate-500 font-mono tracking-wider uppercase opacity-60">
          Built with Cloudflare Stack
        </p>
        
        <button 
          @click="toggleTheme"
          class="group flex items-center justify-center w-9 h-9 rounded-full bg-slate-800/40 hover:bg-slate-700/60 border border-white/5 backdrop-blur-md transition-all relative overflow-hidden dark:bg-white/5 dark:hover:bg-white/10"
