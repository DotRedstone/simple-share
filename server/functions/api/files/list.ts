  ...
  try {
    ...
    if (tab === 'recent') {
      ...
      const result = await db.db
        .prepare(`SELECT * FROM files WHERE user_id = ? AND type != "folder" AND created_at > ? ORDER BY ${sortBy} ${order}`)
      ...
      // 收藏的文件
      const result = await db.db
        .prepare(`SELECT * FROM files WHERE user_id = ? AND starred = 1 ORDER BY ${sortBy} ${order}`)
        .bind(user.userId)
        .all()
      files = result.results as any[]
    } else if (tab === 'shares') {
      ...
      if (fileIds.length > 0) {
        ...
        const result = await db.db
          .prepare(`SELECT * FROM files WHERE id IN (${placeholders}) ORDER BY ${sortBy} ${order}`)
          .bind(...fileIds)
          .all()
        files = result.results as any[]
      }
      ...
      // 所有文件或指定文件夹
      files = await db.getFilesByUserId(user.userId, parentId, sortBy, order)
    }
